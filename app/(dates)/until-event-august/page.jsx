...
const useAugustCountdownTimer = () => {
  ...
  useEffect(() => {
    const calculateTimeLeft = () => {
      ...
      const nextAugust = new Date(today.getFullYear(), 7, 1); // Next August
      if (today > nextAugust) {
        nextAugust.setFullYear(nextAugust.getFullYear() + 1); // Move to next year if August has passed
      }

      let timeUntilAugust = nextAugust - today; // Time until next August in milliseconds

      // Convert milliseconds to days, hours, minutes, seconds
      const days = Math.floor(timeUntilAugust / (1000 * 60 * 60 * 24));
      const hours = Math.floor(
        (timeUntilAugust % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
      );
      const minutes = Math.floor(
        (timeUntilAugust % (1000 * 60 * 60)) / (1000 * 60)
      );
      const seconds = Math.floor((timeUntilAugust % (1000 * 60)) / 1000);
...
const AugustCountdownTimer = () => {
  const timeLeft = useAugustCountdownTimer();

  return (
    <>
      <HeaderModal />

      <TopForm title={"Сколько дней осталось до"} span={"августа"}>
        <Contents>
          <div className="label-row">
            <div className="row-vans-bottom">
              <span className="top-adress">
                <a href="https://boxcalculators.ru/">boxcalculators.ru</a> /
                Сколько дней осталось до августа
              </span>
          ...
          <p>
            ...
            осталось до наступления августа. Этот удобный инструмент производит
            обратный отсчет до 1 августа, а также определяет, сколько осталось
            дней, часов, минут и секунд до начала нового месяца
          </p>
          <p>
            Сколько дней до августа? Этот вопрос интересует многих, особенно
            когда наступает конец июля. Онлайн калькулятор поможет вам точно
            ...
            осталось до 1 августа.
          </p>
          <p>
            Таймер обратного отсчета - это не только удобный инструмент для тех,
            кто ждет августа. Он также может пригодиться, если вы планируете
            важное событие или просто хотите следить за временем. Например, вы
            можете установить таймер до начала августа и следить за тем, сколько
            времени осталось.
          </p>
          <p>
            Важно помнить, что 1 августа - это не только начало нового месяца,
...
export default AugustCountdownTimer;
