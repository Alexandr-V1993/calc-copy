...
const useJulyCountdownTimer = () => {
  ...
  useEffect(() => {
    const calculateTimeLeft = () => {
      ...
      const nextJuly = new Date(today.getFullYear(), 6, 1); // Next July
      if (today > nextJuly) {
        nextJuly.setFullYear(nextJuly.getFullYear() + 1); // Move to next year if July has passed
      }

      let timeUntilJuly = nextJuly - today; // Time until next July in milliseconds

      // Convert milliseconds to days, hours, minutes, seconds
      const days = Math.floor(timeUntilJuly / (1000 * 60 * 60 * 24));
      const hours = Math.floor(
        (timeUntilJuly % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
      );
      const minutes = Math.floor(
        (timeUntilJuly % (1000 * 60 * 60)) / (1000 * 60)
      );
      const seconds = Math.floor((timeUntilJuly % (1000 * 60)) / 1000);
...
const JulyCountdownTimer = () => {
  const timeLeft = useJulyCountdownTimer();

  return (
    <>
      <HeaderModal />

      <TopForm title={"Сколько дней осталось до"} span={"июля"}>
        <Contents>
          <div className="label-row">
            <div className="row-vans-bottom">
              <span className="top-adress">
                <a href="https://boxcalculators.ru/">boxcalculators.ru</a> /
                Сколько дней осталось до июля
              </span>
          ...
          <p>
            ...
            осталось до наступления июля. Этот удобный инструмент производит
            обратный отсчет до 1 июля, а также определяет, сколько осталось
            дней, часов, минут и секунд до начала нового месяца.
          </p>
          <p>
            Сколько дней до июля? Этот вопрос интересует многих, особенно когда
            наступает конец июня. Онлайн калькулятор поможет вам точно
          ...
          <p>
            ...
            кто ждет июля. Он также может пригодиться, если вы планируете важное
            событие или просто хотите следить за временем. Например, вы можете
            установить таймер до начала июля и следить за тем, сколько времени
            осталось.
          </p>
          <p>
            Важно помнить, что 1 июля - это не только начало нового месяца, но и
...
export default JulyCountdownTimer;
