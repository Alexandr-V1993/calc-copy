...
const useOctoberCountdownTimer = () => {
  ...
  useEffect(() => {
    const calculateTimeLeft = () => {
      ...
      const nextOctober = new Date(today.getFullYear(), 9, 1); // Next October
      if (today > nextOctober) {
        nextOctober.setFullYear(nextOctober.getFullYear() + 1); // Move to next year if October has passed
      }

      let timeUntilOctober = nextOctober - today; // Time until next October in milliseconds

      // Convert milliseconds to days, hours, minutes, seconds
      const days = Math.floor(timeUntilOctober / (1000 * 60 * 60 * 24));
      const hours = Math.floor(
        (timeUntilOctober % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
      );
      const minutes = Math.floor(
        (timeUntilOctober % (1000 * 60 * 60)) / (1000 * 60)
      );
      const seconds = Math.floor((timeUntilOctober % (1000 * 60)) / 1000);
...
const OctoberCountdownTimer = () => {
  const timeLeft = useOctoberCountdownTimer();

  return (
    <>
      <HeaderModal />

      <TopForm title={"Сколько дней осталось до"} span={"октября"}>
        <Contents>
          <div className="label-row">
            <div className="row-vans-bottom">
              <span className="top-adress">
                <a href="https://boxcalculators.ru/">boxcalculators.ru</a> /
                Сколько дней осталось до октября
              </span>
          ...
          <p>
            ...
            осталось до наступления октября. Этот удобный инструмент производит
            обратный отсчет до 1 октября, а также определяет, сколько осталось
            дней, часов, минут и секунд до начала нового месяца.
          </p>
          <p>
            Сколько дней до октября? Этот вопрос интересует многих, особенно
            когда наступает конец сентября. Онлайн калькулятор поможет вам точно
            ...
            осталось до 1 октября.
          </p>
          <p>
            Таймер обратного отсчета - это не только удобный инструмент для тех,
            кто ждет октября. Он также может пригодиться, если вы планируете
            важное событие или просто хотите следить за временем. Например, вы
            можете установить таймер до начала октября и следить за тем, сколько
            времени осталось.
          </p>
          <p>
            Важно помнить, что 1 октября - это не только начало нового месяца,
...
export default OctoberCountdownTimer;
