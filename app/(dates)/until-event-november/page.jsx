...
const useNovemberCountdownTimer = () => {
  ...
  useEffect(() => {
    const calculateTimeLeft = () => {
      ...
      const nextNovember = new Date(today.getFullYear(), 10, 1); // Next November
      if (today > nextNovember) {
        nextNovember.setFullYear(nextNovember.getFullYear() + 1); // Move to next year if November has passed
      }

      let timeUntilNovember = nextNovember - today; // Time until next November in milliseconds

      // Convert milliseconds to days, hours, minutes, seconds
      const days = Math.floor(timeUntilNovember / (1000 * 60 * 60 * 24));
      const hours = Math.floor(
        (timeUntilNovember % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
      );
      const minutes = Math.floor(
        (timeUntilNovember % (1000 * 60 * 60)) / (1000 * 60)
      );
      const seconds = Math.floor((timeUntilNovember % (1000 * 60)) / 1000);
...
const NovemberCountdownTimer = () => {
  const timeLeft = useNovemberCountdownTimer();

  return (
    <>
      <HeaderModal />

      <TopForm title={"Сколько дней осталось до"} span={"ноября"}>
        <Contents>
          <div className="label-row">
            <div className="row-vans-bottom">
              <span className="top-adress">
                <a href="https://boxcalculators.ru/">boxcalculators.ru</a> /
                Сколько дней осталось до ноября
              </span>
          ...
          <p>
            ...
            осталось до наступления ноября. Этот удобный инструмент производит
            обратный отсчет до 1 ноября, а также определяет, сколько осталось
            дней, часов, минут и секунд до начала нового месяца.
          </p>
          <p>
            Сколько дней до ноября? Этот вопрос интересует многих, особенно
            когда наступает конец октября. Онлайн калькулятор поможет вам точно
            ...
            осталось до 1 ноября.
          </p>
          <p>
            Таймер обратного отсчета - это не только удобный инструмент для тех,
            кто ждет ноября. Он также может пригодиться, если вы планируете
            важное событие или просто хотите следить за временем. Например, вы
            можете установить таймер до начала ноября и следить за тем, сколько
            времени осталось.
          </p>
          <p>
            Важно помнить, что 1 ноября - это не только начало нового месяца, но
...
export default NovemberCountdownTimer;
