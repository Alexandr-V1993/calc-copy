...
function Shveller({ onDataUpdate }) {
  ...
  useEffect(() => {
    fetch(
      "https://calcoffee.ru/api/metal/standards?type=channel&metal=steel&subtype=equal,unequal"
    )
  ...
  const handleStandardChange = (e) => {
    ...
    if (selectedStandard) {
      ...
      fetch(
        `https://calcoffee.ru/api/metal/specifications?standardId=${selectedStandardId}&type=channel&subtype=equal,unequal`
      )
        .then((response) => response.json())
  ...
  const handleSpecificationChange = (e) => {
    ...
    if (selectedSpecification && diam) {
      ...
      const updatedObj = {
        ...
        length: obj.length,
        a:
          filteredSpecification.a !== null
            ? filteredSpecification.a
            : undefined,
        b:
          filteredSpecification.b !== null
            ? filteredSpecification.b
            : undefined,
        ro: obj.ro,
        t:
          selectedSpecification.s !== null
            ? selectedSpecification.s
            : undefined,
        r:
          selectedSpecification.r !== null
            ? selectedSpecification.r
            : undefined,
      };
  ...
  const handleMaterialChange = (e) => {
    ...
    if (selectedMaterial) {
      const { materialId, ...updatedObj } = obj;
  ...
  const handleWidthChange = (e) => {
    ...
    const updatedObj = {
      ...obj,
      b: newWidth === "" ? null : parseFloat(newWidth),
    };
    setObj(updatedObj);
    onDataUpdate(updatedObj);
  };
  ...
  const handleHeightChange = (e) => {
    ...
    const updatedObj = {
      ...obj,
      h: newHeight === "" ? null : parseFloat(newHeight),
    };
    setObj(updatedObj);
    onDataUpdate(updatedObj);
  };
  ...
  const handleThicknessChange = (e) => {
    ...
    const updatedObj = {
      ...obj,
      a: newThickness === "" ? null : parseFloat(newThickness),
    };
    setObj(updatedObj);
    onDataUpdate(updatedObj);
  };
  ...
  const handleRadiusChange = (e) => {
    ...
    const updatedObj = {
      ...obj,
      r: newRadius === "" ? null : parseFloat(newRadius),
    };
    setObj(updatedObj);
    onDataUpdate(updatedObj);
  };
      ...
      {diam ? (
                ...
                {specifications.map((specification) => (
                  <option key={specification.id} value={specification.id}>
                    {specification.name !== null
                      ? specification.name
                      : `${specification.h}x${specification.b}${
                          specification.a !== null ? `x${specification.a}` : ""
                        }x${specification.t}x${specification.d}`}
                  </option>
                ))}
              </select>
            </label>
          ...
          <div className="calc-frow">
            <div className="calc-fright">
              <figure>
                <img
                  src="/shveller.png"
                  alt="Размеры швеллера"
                  title="Размеры швеллера"
                  loading="lazy"
                  decoding="async"
                  width="150"
                  height="171"
              ...
              <div className="dimensions-container">
                <span className="dimension-s-shvelelr">
                  {obj.s !== null ? (/\d/.test(obj.s) ? obj.s : `s`) : ""}
                </span>
                <span className="dimension-h-shvelelr">
                  {obj.h !== null ? (/\d/.test(obj.h) ? obj.h : `h`) : ""}
                </span>
                <span className="dimension-t-shvelelr">
                  {obj.t !== null ? (/\d/.test(obj.t) ? obj.t : `t`) : ""}
                </span>
                <span className="dimension-b-shvelelr">
        ...
        <div className="flex-column-dev">
          <div className="flex-column-dev">
            ...
            <div className="calc-fleft">
              <label>
                <span>Ширина полки (b)</span>
                <input
                  type="number"
                  name="width"
                  className="calc-inp w-100"
                  onChange={handleWidthChange}
                  placeholder="Ширина B"
                  required
                  step="0.01"
                />
              </label>
            ...
            <div className="calc-fleft">
              <label>
                <span>Радиус скругления (R) для гнутых</span>
                <input
                  type="number"
                  name="radius"
                  className="calc-inp w-100"
