...
function TrubaProfilnaya({ onDataUpdate }) {
  ...
  useEffect(() => {
    fetch(
      "https://calcoffee.ru/api/metal/standards?type=pipe&metal=steel&subtype=square,rectangle"
    )
  ...
  const handleStandardChange = (e) => {
    ...
    if (selectedStandard) {
      ...
      fetch(
        `https://calcoffee.ru/api/metal/specifications?standardId=${selectedStandardId}&type=pipe&subtype=square,rectangle`
      )
        .then((response) => response.json())
  ...
  const handleSpecificationChange = (e) => {
    ...
    if (selectedSpecification && diam) {
      const filteredSpecification = Object.keys(selectedSpecification)
        .filter((key) => !["h", "weight", "name", "id", "d", "s"].includes(key))
        .reduce((obj, key) => {
  ...
  const handleMetalChange = (e) => {
    ...
    fetch(`https://calcoffee.ru/api/metal/materials?metal=${selectedMetal}`)
      ...
      .then((data) => {
        ...
        // Удаляем свойство materialId из объекта obj
  ...
  const handleWidthChange = (e) => {
    ...
    const updatedObj = {
      ...obj,
      a: newWidth === "" ? null : parseFloat(newWidth), // Устанавливаем null, если строка пустая
    };
    setObj(updatedObj);
    onDataUpdate(updatedObj);
  };
  ...
  const handleHeightChange = (e) => {
    ...
    const updatedObj = {
      ...obj,
      b: newHeight === "" ? null : parseFloat(newHeight), // Устанавливаем null, если строка пустая
    };
    setObj(updatedObj);
    onDataUpdate(updatedObj);
  };
  ...
  const handleThicknessChange = (e) => {
    ...
    const updatedObj = {
      ...obj,
      t: newThickness === "" ? null : parseFloat(newThickness), // Устанавливаем null, если строка пустая
    };
    setObj(updatedObj);
    onDataUpdate(updatedObj);
  };
                ...
                {specifications.map((specification) => (
                  <option key={specification.id} value={specification.id}>
                    {`${specification.a !== null ? specification.a : ""}${
                      ...
                      specification.s !== null ? `x${specification.s}` : ""
                    }`}{" "}
                  </option>
                ))}
              </select>
          ...
          <div className="calc-frow">
            <div className="calc-fright">
              <figure>
                <img
                  src="/trubaprof.png"
                  alt="Размеры профильной трубы"
                  title="Размеры профильной трубы"
                  loading="lazy"
                  decoding="async"
                  width="150"
                  height="171"
              ...
              <div className="dimensions-container">
                <span className="dimension-b-trubapro">
                  {obj.b !== null && /\d/.test(obj.b)
                    ? obj.b
                    : obj.a !== null && /\d/.test(obj.a)
                    ? obj.a
                    : "b"}
                </span>

                <span className="dimension-a-trubapro">
                  {obj.a !== null ? (/\d/.test(obj.a) ? obj.a : `a`) : ""}
                </span>
                <span className="dimension-t-trubapro">
...
export default TrubaProfilnaya;
