...
function List({ onDataUpdate }) {
  ...
  useEffect(() => {
    fetch("https://calcoffee.ru/api/metal/standards?type=sheet&metal=steel")
  ...
  const handleStandardChange = (e) => {
    ...
    if (selectedStandard) {
      ...
      fetch(
        `https://calcoffee.ru/api/metal/specifications?standardId=${selectedStandardId}&type=sheet`
      )
        .then((response) => response.json())
  ...
  const handleSpecificationChange = (e) => {
    ...
    if (selectedSpecification && diam) {
      ...
      if (updatedObj.s === null) {
        updatedObj.s = updatedObj.a;
      }
  ...
  const handleMetalChange = (e) => {
    ...
    fetch(`https://calcoffee.ru/api/metal/materials?metal=${selectedMetal}`)
      ...
      .then((data) => {
        ...
        onDataUpdate({ ...obj, ro: null }); // Очищаем ro, когда меняем металл
      })
        ...
        <>
              ...
              >
                ...
                {specifications.map((specification) => (
                  <option key={specification.id} value={specification.id}>
                    {specification.a}x{specification.s}
                  </option>
                ))}
              </select>
            </label>
          ...
          <div className="calc-fright">
            <dl className="m-0">
              ...
              <dt className="font-dt">Длина</dt>
              <dd className="calc-input-desc centre-calcs">
          ...
          <div className="calc-frow">
            <div className="calc-fright">
              <figure>
                <img
                  src="/list.png"
                  alt="Размеры стального листа"
                  title="Размеры стального листа"
                  loading="lazy"
                  decoding="async"
                  width="150"
                  height="171"
